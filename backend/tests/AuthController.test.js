...
describe('AuthController - Testes Unitários (CT01 a CT04)', () => {
  ...
  describe('1. Registro de Conta (Gestão de Multi-tenancy)', () => {
    ...
    test('CT01 (Positivo): Deve registrar empresa e usuário com sucesso', async () => {
      ...
      Tenant.create.mockResolvedValue({ id: 'tenant-123', name: 'Empresa Teste', update: jest.fn() });
      bcrypt.genSalt.mockResolvedValue('salt');
      bcrypt.hash.mockResolvedValue('hashed_password');
      User.create.mockResolvedValue({ id: 'user-123', name: 'Admin Teste', email: 'admin@teste.com' });
      ...
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ message: 'Empresa e usuário criados com sucesso!' })
      );
    });
    ...
    test('CT02 (Negativo): Deve retornar erro se faltar dado obrigatório no Sequelize', async () => {
      ...
      User.findOne.mockResolvedValue(null);
      Tenant.create.mockRejectedValue(new Error('notNull Violation: Tenant.name cannot be null'));
  ...
  describe('2. Autenticação de Usuários (Login)', () => {
    ...
    test('CT03 (Positivo): Deve fazer login com credenciais corretas e retornar JWT', async () => {
      // ARRANGE
      const req = { body: { email: "admin@teste.com", password: "senha_correta" } };
      const res = mockResponse();

      const mockUser = { id: 'user-1', name: 'Admin', email: 'admin@teste.com', tenant_id: 'tenant-1', password_hash: 'hash' };
    ...
    test('CT04 (Negativo): Deve bloquear login com senha incorreta', async () => {
      // ARRANGE
      const req = { body: { email: "admin@teste.com", password: "senha_errada" } };
      const res = mockResponse();
